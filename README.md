...
바쁜 일상 속에서 새로운 취미를 시작하고 싶어도, 정작 어떤 취미가 나에게 맞는지, 같은 취미를 가진 사람을 어디서 만날 수 있는지 알기 어려운 경우가 많습니다.

**HOBEE**는 이런 고민에서 출발한 **취미 기반 커뮤니티 플랫폼**입니다. 적성 검사로 나에게 맞는 취미를 추천받고, 취미·자유·공지·모임 모집 게시판에서 같은 관심사를 가진 사람들과 자유롭게 소통할 수 있습니다. 팔로우, 실시간 채팅, 모임 모집, 챌린지 등의 소셜 기능을 통해 취미 친구를 만들고 함께 성장하는 경험을 제공합니다.

본 레포지토리는 팀 프로젝트로 개발한 HOBEE를 바탕으로, 팀 프로젝트 종료 이후 개인 리팩토링을 통해 코드 품질을 개선하고 팔로우·알림·채팅·챌린지 등의 신규 기능을 직접 설계·구현한 결과물입니다.
...
- **프로필 페이지** *(리팩토링 추가)* : 다른 회원의 프로필·게시글·챌린지 참여 내역 확인, 팔로우·채팅 버튼
- **프로필 이미지 즉시 업로드** *(리팩토링 추가)* : 파일 선택 즉시 서버 업로드 + 미리보기, 기본 이미지로 변경 기능
...
<details>
<summary><b>10개의 질문 HTML 파일 코드 중복 문제</b></summary>
...
<details>
<summary><b>점수 10배 이벤트 오류 (배열 순회 문제)</b></summary>
...
<details>
<summary><b>점수가 Map의 키 값으로 사용되어 동점 시 덮어쓰기 발생</b></summary>
...
<details>
<summary><b>게시글·댓글 신고가 동일 모달을 공유하는 문제</b></summary>
...
<details>
<summary><b>FreeBoardController·HobbyBoardController·NoticeBoardController 중복 코드 문제</b></summary>
...
<details>
<summary><b>파일 저장 실패 시 DB 불일치 문제</b></summary>
