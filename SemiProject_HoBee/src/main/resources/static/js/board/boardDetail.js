...
function showReport(e) {
	...
	commentAuthorLevel = e.currentTarget.dataset.commentAuthorLevel;
...
document.querySelector(".submit-report-btn").addEventListener("click", async () => {
	...
	if (boardAuthorLevel == 2 || commentAuthorLevel == 2) {
		alert("관리자 계정은 신고할 수 없습니다.")
		resetReport();
		return;
	}

	let today = StringToDate(new Date());

	const count = await fetch("/report/todayReportCount?memberNo=" + loginMemberNo + "&today=" + today)
		.then(resp => resp.text());

	if (count >= 5) {
		alert("계정 당 하루 최대 5회까지 신고할 수 있습니다.")
		resetReport();
		return;
	}
	...
	const result = await fetch("/report/insertReport", {
		method: "POST",
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify(obj)
	})
...
if (updateBtn != null) { // 수정 버튼 존재 시
	...
	updateBtn.addEventListener("click", () => {
		...
		if (categoryCode != null) {
			url = `/editBoard/${boardCode}/${categoryCode}/${boardNo}/update`
		} else {
